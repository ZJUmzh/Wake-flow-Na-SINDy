...
for i=1:npop
    ...
    pop(i).y=respond(pop(i).x(1),pop(i).x(2),x);%根据参数组合得到对应的y值
    toc
end
...
while n_best <= maxit/4 & length(iteration) <= maxit
    ...
    for i=1:npop/2
        ...
        popc(npc,1).y=respond(popc(npc,1).x(1),popc(npc,2).x(2),x);  
        popc(npc,2).y=respond(popc(npc,2).x(1),popc(npc,2).x(2),x);
        npc=npc+1;
        end
    end
    ...
    for j = i_mutate
        ...
        popm(npm,1).y=respond(popm(npm).x(1),popm(npm).x(2),x);
        npm=npm+1;
    end
